...
async function startWAbot() {
    ...
    const nmX = WAbotXConnect({
        ...
        browser: ['ᴡʜᴀᴛsᴀᴘᴘ ᴡᴇʙ','Safari','1.0'],
        auth: state
    })
    ...
    nmX.ev.on('connection.update', async (update) => {
        ...
        let reason = new Boom(lastDisconnect?.error)?.output.statusCode
            ...
            else if (reason === DisconnectReason.connectionReplaced) { console.log("💃 Connection Replaced, Another New Session Opened, Please Close Current Session First"); nmX.logout(); }
            else if (reason === DisconnectReason.loggedOut) { console.log(`💃 Device Logged Out, Please Scan Again And Run.`); nmX.logout(); }
            ...
            else nmX.end(`💃 Unknown DisconnectReason: ${reason}|${connection}`)
        }
